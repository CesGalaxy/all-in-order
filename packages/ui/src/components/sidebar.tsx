...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
      ...
      // This sets the cookie to keep the sidebar state.
          document.cookie = `${openState.map((sidebarName) => `${SIDEBAR_COOKIE_NAME}:${sidebarName}=${openState.includes(sidebarName)}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`).join("; ")}`
    },
    [setOpenProp, open]
  )
...
function SidebarTrigger({
  ...
  onClick,
  ...props
}: Omit<React.ComponentProps<typeof Button>, "name"> & {name: string | string[]}) {
  ...
  return (
    <Button
      ...
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar(name ? (typeof name === "string" ? [name] : name) : [])
      }}
      {...props}
    >
      <PanelLeftIcon />
...
function SidebarRail({ name, className, ...props }: React.ComponentProps<"button"> & {name: string}) {
  const { toggleSidebar } = useSidebar()
