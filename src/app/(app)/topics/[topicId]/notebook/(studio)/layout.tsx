...
import getNotionPages from "@/modules/notebook/notion/lib/getNotionPages";
...
export default async function Layout({ params, children }: Readonly<{
    ...
    const { data, files } = nb;
    const { data: notionPages } = await getNotionPages(data.id);

    return <NotebookProvider
        topicId={parseInt(topicId)}
        initialPages={files}
        initialData={data}
        initialNotionPages={notionPages}
        userId={user.id}
    >
